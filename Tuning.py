...
for tun_param in np.linspace(0.1,0.9,9):
    ...
    # _, ga_best_fitness, _ = mlr.algorithms.genetic_alg(problem=problem, max_iters=2048, pop_size=200, mutation_prob=tun_param, random_state=random_seed)
    ...
    _, mi_best_fitness, _ = mlr.algorithms.mimic(problem=problem, max_iters=512, pop_size=400, keep_pct=tun_param, random_state=random_seed)
    fitness.append(mi_best_fitness)

plt.plot(x_ax, fitness)

plt.title("Keep Percent Tuning Curve")
plt.show()
