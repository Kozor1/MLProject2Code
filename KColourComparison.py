...
for n in range(10,size,10):
    ...
    problem = mlr.generators.max_k_color_generator.MaxKColorGenerator.generate(seed=random_seed, number_of_nodes=n, max_colors=6)

    rhc_start_time = time.time()
    _, rhc_best_fitness, _ = mlr.algorithms.random_hill_climb(problem=problem, max_iters=512, restarts=50, random_state=random_seed)
    rhc_time = time.time() - rhc_start_time
    rhc_fitness.append(rhc_best_fitness)
    rhc_times.append(rhc_time)

    sa_start_time = time.time()
    _, sa_best_fitness, _ = mlr.algorithms.simulated_annealing(problem=problem, max_iters=8192, schedule=mlr.algorithms.decay.GeomDecay(init_temp=1.0), random_state=random_seed)
    sa_time = time.time() - sa_start_time
    sa_fitness.append(sa_best_fitness)
    sa_times.append(sa_time)

    ga_start_time = time.time()
    _, ga_best_fitness, _ = mlr.algorithms.genetic_alg(problem=problem, max_iters=2048, pop_size=50, mutation_prob=0.40, random_state=random_seed)
    ga_time = time.time() - ga_start_time
    ga_fitness.append(ga_best_fitness)
    ga_times.append(ga_time)

    mi_start_time = time.time()
    _, mi_best_fitness, _ = mlr.algorithms.mimic(problem=problem, max_iters=512, pop_size=400, keep_pct=0.25, random_state=random_seed)
