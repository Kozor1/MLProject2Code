...
for n in range(3, size):
    ...
    problem = mlr.generators.queens_generator.QueensGenerator.generate(seed=random_seed, size=n)

    rhc_start_time = time.time()
    _, rhc_best_fitness, _ = mlr.algorithms.random_hill_climb(problem=problem, max_iters=512, restarts=40, random_state=random_seed)
    rhc_time = time.time() - rhc_start_time
    rhc_fitness.append(rhc_best_fitness)
    rhc_times.append(rhc_time)

    sa_start_time = time.time()
    _, sa_best_fitness, _ = mlr.algorithms.simulated_annealing(problem=problem, max_iters=8192, schedule=mlr.algorithms.decay.GeomDecay(init_temp=100.0), random_state=random_seed)
    ...
    _, mi_best_fitness, _ = mlr.algorithms.mimic(problem=problem, max_iters=512, pop_size=450, keep_pct=0.33, random_state=random_seed)
